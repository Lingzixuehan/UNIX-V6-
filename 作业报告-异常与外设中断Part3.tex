...
\textbf{重要说明}：经过实际测试发现，在Unix V6++系统中，\textbf{整数除零可能不会触发SIGFPE信号}。这是因为：
\begin{itemize}
    \item SIGFPE原本是"Floating Point Exception"，主要用于浮点运算异常
    \item 不同架构和系统对整数除零的处理方式不同
    \item Unix V6++可能没有完全实现整数除零的信号触发机制
\end{itemize}

\subsubsection{手动检查方案}

由于信号机制可能不可靠，我们采用\textbf{双重保护}策略：
...
\begin{enumerate}[itemsep=0.5em]
    \item 注册 SIGFPE 信号处理函数（展示信号处理机制，即使可能不触发）
    \item 进入主循环
    \item 接收用户输入的被除数 \texttt{a} 和除数 \texttt{b}
    \item \textbf{手动检查 b 是否为 0}（实际使用的保护机制）
    \item 如果 \texttt{b = 0}，输出错误信息并继续循环
    \item 如果 \texttt{b ≠ 0}，执行除法运算 \texttt{c = a / b}
    \item 输出计算结果
    \item 程序继续运行，等待下一次输入
\end{enumerate}
...
\begin{itemize}
    ...
    \item 提供友好的用户交互界面
    \item 使用 \texttt{gets()} 和自定义的 \texttt{str\_to\_int()} 函数处理输入（因为 Unix V6++ 不支持 \texttt{scanf}）
    \item \textbf{使用手动检查确保除数不为0}（双重保护）
\end{itemize}

\subsubsection{输入处理实现}

由于 Unix V6++ 系统不提供 \texttt{scanf} 和 \texttt{atoi} 函数，我们需要自己实现字符串到整数的转换：
...
\subsubsection{有限的标准库函数}

\begin{itemize}
    \item 不支持 \texttt{scanf}、\texttt{atoi} 等现代标准库函数
    \item 只提供基本的 \texttt{gets}、\texttt{printf} 等函数
    \item 需要开发者自己实现部分功能
    \item 体现了早期 Unix 系统的简洁设计哲学
\end{itemize}
...
\subsection{实现挑战与解决方案}

\subsubsection{缺少标准库函数}

\begin{itemize}[itemsep=0.5em]
    \item \textbf{问题}：Unix V6++ 不支持 \texttt{scanf} 和 \texttt{atoi} 函数
    \item \textbf{解决}：自行实现 \texttt{str\_to\_int} 函数进行字符串到整数的转换
    \item \textbf{收获}：深入理解标准库函数的底层实现原理
\end{itemize}

\subsubsection{字符串处理}

\begin{itemize}[itemsep=0.5em]
    \item 使用 \texttt{gets} 函数读取整行输入
    \item 手动解析字符串，支持正负数和空格处理
    \item 体会到底层编程的复杂性和挑战
\end{itemize}
